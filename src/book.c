...
static char BOOKFILE[300]="db/mainbook.bin";
static char TOERFILE[300]="db/tourbook.bin";
static char EPD[200]="rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -";
static char FROM[10],TO[10];
static char ERROR=0;
...
static long HK1,HK2;
static int  XD,XE,AZ,RND;
static char FROM1[20],FROM2[20],TO1[20],TO2[20];
static unsigned char _GB[100000];                   // TOERBOEK.BIN
static int  BOEKSTAT[20];
...
{       int x,d,e;                                                          // DEMONSTRATION
        ...
        strcpy(BOOKFILE,"mainbook.bin");                                    // main book to search

        INITIALIZE();                                                       // Initialize (one time call)
        if (ERROR)
         { printf("Missing obliged file(s) RANDOM1.BIN and/or RANDOM2.BIN"); exit(1); }

        strcpy(EPD,"rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -"); // start position
        FIND_OPENING();
        if (ERROR) printf("Something went wrong, error-code %d",ERROR);
         else printf("Move: %s-%s\nList: ",FROM,TO);
        for (x=0; x<AZ; x++) printf("%c%c-%c%c ",FROM1[x],FROM2[x],TO1[x],TO2[x]);
        printf("\n\n");
        ...
        strcpy(EPD,"rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq e6"); // position after 1.e4 e5
        FIND_OPENING();
...
void INITIALIZE()                                               // read hashkeys in memory

{       int x,y,r; FILE *fp1; FILE *fp2;

        ERROR=0;

        fp1=fopen("db/random1.bin","rb"); if (fp1==NULL) { fclose(fp1); ERROR=1; return; }
        fp2=fopen("db/random2.bin","rb"); if (fp2==NULL) { fclose(fp2); ERROR=1; return; }
        ...
        if (fp1==NULL) { fclose(fp1);                         // no tournament book
...
void FIND_OPENING()                                 // input : BOOKFILE / EPD
                                                    // output: FROM and TO as E2 and E4 and ERROR
{       int xl,px,xx,r,x,y,rnd,bhk1,bhk2,xd,xe;
        int ch,by,v,kleur,boekfase;
        char db[100],eb[100];
        unsigned char zcd[200],zce[200],zcv[200],vb[100];
        char fentab[]   = "??PNBRQKpnbrqk??";
        char fenkleur[] = "wb??";

        ERROR=0;

        if ((FPB = fopen(BOOKFILE,"rb")) == NULL) { ERROR=2; fclose(FPB); return; }

        FROM[0]=0; TO[0]=0;
        db[0]=0; eb[0]=0; vb[0]=255;

        RND=clock();                             // initialize randomizer
        clock_t a; a=clock(); srand(time(NULL));
        ...
        if (_GB[0]==255) boekfase=1;                // no tournament book selected -> search main book
...
boek12: if (boekfase)                                       // read main book
         ...
         { xl=xl+2; if (_GB[xl]==255 && _GB[xl+1]==255) goto boek90;  // read tournament book
